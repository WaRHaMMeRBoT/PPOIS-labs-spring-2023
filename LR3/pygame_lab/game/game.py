...
from pygame_lab.game.alien import *
from pygame_lab.game.menu import Menu
from pygame_lab.game.player import Player
from pygame_lab.game.weapon import Laser
...
class Game:
    ...
    def victory(self):
        ...
        game_victory1 = font.render(f"Congratulations. You won and scored {self.player_sprite.score} points.", False,
                                    'white')
    ...
    def game_over(self):
        ...
        table = Table()
        cur_table = table.load_table()
        player = f"player{random.randint(1,30)}"
        while cur_table.get(player):
            player = f"player{random.randint(1, 15)}"
        table.upload_table(info={player: self.player_sprite.score})
