...
class Player(pygame.sprite.Sprite):
    ...
    def shoot(self, wave):
        if wave < 10:
            self.weapon.add(Laser(self.rect.center, speed_multiplayer=2.5))
            self.shoot_cooldown = Laser(self.rect.center).player_cooldown
        elif wave >= 10 and wave < 15:
            self.weapon.add(DoubleLaser(self.rect.center, speed_multiplayer=2.5))
            self.shoot_cooldown = DoubleLaser(self.rect.center).player_cooldown
        elif wave >= 15:
            self.weapon.add(HighDamageLaser(self.rect.center, speed_multiplayer=2.5))
