...
class Game(object):
    def __init__(self):
        ...
        self.menu = Menu(("Старт", "Таблица рекордов", "Как играть", "Выход"), font_color=WHITE, font_size=60)
    ...
    def record_box(self, screen):
        ...
        while not done:
            ...
            self.display_message(screen, "Поздравляю! Вы вошли в топ-3 лучших! Введите ваше имя!")
    ...
    def display_frame(self, screen):
        ...
        if self.game_over:
            if self.about:
                self.display_message(screen, "Это аркадная игра \"Pacman\"")
                self.display_messagepos(screen,"Управление: Leftkey, rightkey, upkey, downkey", x=SCREEN_WIDTH/2,y=SCREEN_HEIGHT/2 + 30)
                self.display_messagepos(screen, "Для победы необходимо собирать точки и избегать врагов.", x=SCREEN_WIDTH / 2,
    ...
    def display_messagepos(self, screen, message, color=(255, 255, 0), x=int, y=int):
        label = self.font.render(message, True, color)
        width = label.get_width()
        height = label.get_height()
        posX = x - (width / 2)
...
class Menu(object):
    ...
    def __init__(self, items, font_color=(0, 0, 0), select_color=(255, 255, 0), ttf_font=None, font_size=25):
