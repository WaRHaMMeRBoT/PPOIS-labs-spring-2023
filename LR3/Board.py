...
class Board:
    ...
    def checkPoint(self, x, y, colorNum, visited):
        if (self.grid[x][y]!=-1):
            if (colorNum==self.grid[x][y].colorNum and not (x,y) in visited):
                return True

    def floodFillDel(self, x, y, visited):
        newVis = visited.copy()
        newVis.append((x,y))
    ...
    def checkSwap(self, x1, y1, x2, y2):
    ...
    def startSwap(self, x1, y1, x2, y2, isHor):
