...
class Event(object):
    ...
    def start_win(self, core):
        ...
        core.get_map().get_player().xVel = 1
    ...
    def update(self, core):
        if self.type == 0:
            self.yVel += GRAVITY * FALL_MULTI if self.yVel < 6 else 0
            core.get_map().get_player().rect.y += self.yVel
            ...
            if not self.player_in_castle:
                ...
                if core.get_map().time > 0:
                    ...
                    if self.time == 0:
                        ...
                        core.get_mm().currentGameState = 'Loading'
                        core.get_mm().oLoadingMenu.set_text_and_type('', False)
                        core.get_mm().oLoadingMenu.update_time()
                        core.get_sound().play('game_over', 0, 0.5)
