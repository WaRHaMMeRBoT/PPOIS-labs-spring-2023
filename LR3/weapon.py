...
class Weapon:
    ...
    def shoot(self, player):
        if self.ammo_count > 0:
            if self.type == 'gun':
                Bullet(player.screen, player.x, player.y,
                       player.angle, self.damage, self.ammo_speed)

            if self.type == 'shotgun':
                Bullet(player.screen, player.x, player.y,
                       player.angle, self.damage, self.ammo_speed)
                Bullet(player.screen, player.x, player.y,
                       player.angle + 3, self.damage, self.ammo_speed)
                Bullet(player.screen, player.x, player.y,
                       player.angle - 3, self.damage, self.ammo_speed)
                Bullet(player.screen, player.x, player.y,
                       player.angle + 6, self.damage, self.ammo_speed)
                Bullet(player.screen, player.x, player.y,
                       player.angle - 6, self.damage, self.ammo_speed)
                self.ammo_count -= 1
            ...
            if self.type == 'machine':
                ...
                Bullet(player.screen, player.x, player.y,
                       angle, self.damage, self.ammo_speed)
                self.ammo_count -= 1
    ...
    def draw_reload(self, screen, x, y):
        reload_percentage = int(self.reload_cooldown /
                                self.reload_cooldown_max * 359)

        pygame.gfxdraw.arc(screen, int(x), int(y), 22, -90,
                           reload_percentage - 90, (255, 255, 255))
        pygame.gfxdraw.arc(screen, int(x), int(y), 23, -90,
                           reload_percentage - 90, (255, 255, 255))

    def draw_ammo(self, x, y):
        if self.type != 'gun':
            menu.print_text(f'{self.ammo_count}/{self.ammo_count_max}', x + 18, y + 18,
