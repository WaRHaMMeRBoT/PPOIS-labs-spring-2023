...
class Player(object):
    ...
    def player_physics(self, core):
        ...
        if not (core.keyR or core.keyL):
            if self.xVel > 0:
                self.xVel -= SPEED_DEC_RATE
            elif self.xVel < 0:
                self.xVel += SPEED_DEC_RATE
        else:
            if self.xVel > 0:
                if self.fast_moving:
                    if self.xVel > MAX_FASTMOVE_SPEED:
                        self.xVel = MAX_FASTMOVE_SPEED
                else:
                    if self.xVel > MAX_MOVE_SPEED:
                        self.xVel = MAX_MOVE_SPEED
            if self.xVel < 0:
                if self.fast_moving:
                    if (-self.xVel) > MAX_FASTMOVE_SPEED: self.xVel = -MAX_FASTMOVE_SPEED
                else:
                    if (-self.xVel) > MAX_MOVE_SPEED:
                        self.xVel = -MAX_MOVE_SPEED
        if 0 < self.xVel < SPEED_DEC_RATE:
            self.xVel = 0
        if 0 > self.xVel > -SPEED_DEC_RATE:
            self.xVel = 0
        if not self.on_ground:
            # Moving up, button is pressed
            if (self.yVel < 0 and core.keyU):
                self.yVel += GRAVITY
            elif (self.yVel < 0 and not core.keyU):
                self.yVel += GRAVITY * LOW_JUMP_MULTIPLIER
            else:
        ...
        if self.powerLVL in (0, 1, 2):
            if self.xVel == 0:
                ...
                self.spriteTick = 0
                    ...
                    ((self.xVel > 0 and core.keyR and not core.keyL) or
                     (self.xVel < 0 and core.keyL and not core.keyR)) or
                    (self.xVel > 0 and not (core.keyL or core.keyR)) or
                    (self.xVel < 0 and not (core.keyL or core.keyR))
            ):
                ...
                if self.spriteTick <= 10:
                    ...
                    self.spriteTick = 0
                    self.set_image(1)
            elif (self.xVel > 0 and core.keyL and not core.keyR) or (self.xVel < 0 and core.keyR and not core.keyL):
                self.set_image(7)
                self.spriteTick = 0
            if not self.on_ground:
                self.spriteTick = 0
    ...
    def flag_animation_move(self, core, walk_to_castle):
        if walk_to_castle:
            self.direction = True
            if not self.on_ground:
                self.yVel += GRAVITY if self.yVel <= MAX_FALL_SPEED else 0
            x = self.rect.x // 32
            y = self.rect.y // 32
