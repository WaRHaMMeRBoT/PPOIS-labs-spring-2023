...
def read_entities_from(path_to_file):
...
def options():
    while True:
        ...
        OPTIONS_TEXT = get_font(12).render("You have 3 lives to begin with (displayed at top left).", True, "Black")
        OPTIONS_RECT = OPTIONS_TEXT.get_rect(center=(400, 100))
        SCREEN.blit(OPTIONS_TEXT, OPTIONS_RECT)

        OPTIONS_TEXT = get_font(12).render("Each time a rock hits your spaceship, you lose a life.", True, "Black")
        OPTIONS_RECT = OPTIONS_TEXT.get_rect(center=(400, 150))
        SCREEN.blit(OPTIONS_TEXT, OPTIONS_RECT)

        OPTIONS_TEXT = get_font(12).render("You have to avoid rocks.", True, "Black")
...
def main_menu():
    while True:
        ...
        PLAY_BUTTON = Button(image=pygame.image.load("assets/Play Rect.png"), pos=(400, 220),
...
class MyGame(object):
    ...
    PLAYING, DYING, GAME_OVER, STARTING, \
        WELCOME = range(5)
    ...
    def __init__(self):
        ...
        self.width = game_states["WIDTH"]
        self.height = game_states["HEIGHT"]
...
pygame.init()
game_states = read_entities_from("config.json")
SCREEN = pygame.display.set_mode((game_states["WIDTH"], game_states["HEIGHT"]))
