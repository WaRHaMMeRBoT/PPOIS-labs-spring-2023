...
class SportsmenHandler(xml.sax.handler.ContentHandler):
    ...
    def endElement(self, name):
        if self.current == "name":
            ...
            self.sportsman_list[int(self.pos_counter)].append(self.achievements)
        elif self.current == "sports_discipline":
            self.sportsman_list[int(self.pos_counter)].append(self.sports_discipline)
        elif self.current == "rank":
...
def sax_parser_of_save()->list:
    ...
    obj.parse('E:\PPOIS_LR2\save.xml')
