...
class Student:
    ...
    __validated_filters: dict = {'student_name': '', 'student_group': 'none', 'kind': 'none', 'low_limit': '', 'high_limit': ''}
    ...
    def set_up_filters(cls, filters: dict):
        ...
        # ['student_name', 'student_group', 'kind', 'low_limit', 'high_limit']
    ...
    def __selection_of_suitable_values_by_clock(cls, elem, low_high: bool):  # low_high == True -> high, else False
        if cls.__validated_filters['kind'] == 'по болезни':
            sum_of_hours = int(elem[1].community_service[0])
        elif cls.__validated_filters['kind'] == 'по причине':
            sum_of_hours = int(elem[1].community_service[1])
        elif cls.__validated_filters['kind'] == 'без причины':
            sum_of_hours = int(elem[1].community_service[2])
        elif cls.__validated_filters['kind'] == 'по общему количеству':
