...
from erorrs.DateIsNotValid import DateIsNotValid
...
class MyApp(App):
    ...
    def delete(self, instance):
        popup = Popup(title='Variants of delete', size=(SIZE.get('width') / 10, SIZE.get('height') / 10))
    ...
    def search(self, instance):
        popup = Popup(size=(SIZE.get('width')/10, SIZE.get('height')/10), title='Variants of search')
    ...
    def build(self):
        self.label = Label()
        self.print_info()
        popup = Popup(title='Info', content=self.label, size_hint=(1, 1))
        ...
        main_buttons = BoxLayout(orientation='horizontal', size_hint=(1, 0.1), spacing=10, padding=10)
