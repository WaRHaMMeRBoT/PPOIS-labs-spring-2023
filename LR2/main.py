...
from src.DAO.PetDAO import PetDAO
from src.errors.DateIsNotValid import DateIsNotValid
...
class MyWidget(GridLayout):
    ...
    def selected(self, filename):
        try:
            ...
            print(len(petDAO.get_first_page()))
...
class MyApp(App):
    ...
    def add_pet(self,  popup, add_popup: BoxLayout, instance):
        ...
        pet.birth_date = datetime.datetime.strptime(add_popup.children[5].text, '%m.%d.%Y').date()
        pet.date_of_last_admission = datetime.datetime.strptime(add_popup.children[4].text, '%m.%d.%Y').date()
