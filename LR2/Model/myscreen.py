    ...
    def write_to_file(self, path: str):
        ...
        if self.create_empty_file(path):
            ...
            for row in self.table.row_data:
                ...
                data_dict["address"] = row[1]
                data_dict["date_of_birth"] = row[2]
                data_dict["date_of_visit"] = row[3]
                data_dict["doctor_name"] = row[4]
                data_dict["diagnosis"] = row[5]
    ...
    def select_stock_by_filters(self, filters: list):
        ...
        for row in self.table.row_data:
            ...
            if filters[0]:  # product
                ...
                if not (row[4] == filters[4]) or (filters[4] not in row[4]):
                    not_filtered_stock.append(tuple(row))
                    print(len(not_filtered_stock))
                    continue
