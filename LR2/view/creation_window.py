...
class CreationWindow:
    ...
    def __create_entries_fields(self):
        ...
        self.entries.append(a)
        tk.Label(self.creation_window, text=f'Пропуски по болезни').grid(row=2, column=0)
        a = tk.Entry(self.creation_window)
        a.grid(row=2, column=1)
        self.entries.append(a)
        tk.Label(self.creation_window, text=f'Пропуски по уважительной причине').grid(row=3, column=0)
        a = tk.Entry(self.creation_window)
        a.grid(row=3, column=1)
        self.entries.append(a)
        tk.Label(self.creation_window, text=f'Пропуски без уважительной причины').grid(row=4, column=0)
        a = tk.Entry(self.creation_window)
        a.grid(row=4, column=1)
        self.entries.append(a)

    def __save_entry(self):
        dict_of_services = {i + 1: self.entries[i + 2].get() for i in range(3)}
    ...
    def __init__(self):
        ...
        tk.Button(self.creation_window, text='Создать новую запись', command=self.__save_entry).grid(row=5, column=1)
