...
from file_handling.xml_reader import XMLReader
from file_handling.xml_writer import XMLWriter
...
class MainWindow:
    ...
    def __create_tables(self):
        """Таблица"""
        columns = (
            'id', 'stud_name', 'group', 'except_ill', 'except_reason', 'except_without')
        ...
        self.__main_table.heading(f'except_ill', text="По болезни")
        self.__main_table.column(f'except_ill', width=30)
        self.__main_table.heading(f'except_reason', text="По причине")
        self.__main_table.column(f'except_reason', width=30)
        self.__main_table.heading(f'except_without', text="Без причины")
        self.__main_table.column(f'except_without', width=30)
