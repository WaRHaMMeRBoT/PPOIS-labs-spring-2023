...
class FilteringWindow:
    ...
    def __create_filter_fields(self):
        ...
        tk.Label(self.filtering_window, text=f'Фильтрация по виду пропуска').grid(row=2, column=0, sticky=E)
        groups = Student.list_of_groups
        combo = ttk.Combobox(self.filtering_window, values=['по болезни', 'по причине', 'без причины', 'по общему количеству'])
        combo.set("none")
        combo.config(width=19)
        combo.grid(row=2, column=1)
        self.__filter_entries.append(combo)

        tk.Label(self.filtering_window, text=f'Количество пропусков (нижний предел)').grid(row=3, column=0, sticky=E)
        upper_limit = tk.Entry(self.filtering_window, )
        upper_limit.grid(row=3, column=1)
        self.__filter_entries.append(upper_limit)

        tk.Label(self.filtering_window, text=f'Количество пропусков (верхний предел)').grid(row=4, column=0, sticky=E)
        lower_limit = tk.Entry(self.filtering_window, )
        lower_limit.grid(row=4, column=1)
    ...
    def __save_filters(self):
        ...
        names = ['student_name', 'student_group', 'kind', 'low_limit', 'high_limit']
    ...
    def __init__(self):
        ...
        self.filtering_window.geometry('470x140')

        self.__create_filter_fields()
        tk.Button(self.filtering_window, text='Задать фильтры', command=self.__save_filters).grid(row=5, column=1)
