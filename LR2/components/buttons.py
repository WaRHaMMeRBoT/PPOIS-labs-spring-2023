...
def marks_button_layout(controller) -> MDBoxLayout:
    ...
    add_button.bind(on_press=controller.add_mark_dialog)

    calculate_button = MDFloatingActionButton(
        icon="calculator",
        type="small",
        theme_icon_color="Custom",
        elevation=0,
    ...
    calculate_button.bind(on_press=controller.show_gpa_dialog)

    buttons.add_widget(back_button)
    buttons.add_widget(add_button)
    buttons.add_widget(edit_button)
    buttons.add_widget(calculate_button)
    return buttons
