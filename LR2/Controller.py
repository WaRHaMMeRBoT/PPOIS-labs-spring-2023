...
class Core(MDApp):
    ...
    def search_process(self):
        search_attributes = self.collect_delete_attributes(
    ...
    def drop_down_line_up_list(self):
        if not self.line_up_list:
            ...
            self.line_up_list.bind(
                on_press=lambda x: self.drop_down_line_up.open())
    ...
    def set_rank(self, rank: str):
        print(self.rank_item, 'rank_item')
    ...
    def drop_down_achievement_list(self):
        if not self.achievement_list:
            ...
            self.achievement_list.bind(
                on_press=lambda x: self.drop_down_achievement.open())
    ...
    def build(self):
        table_layout = AnchorLayout(
            anchor_x='left', anchor_y='center', size_hint=(1, 1))
