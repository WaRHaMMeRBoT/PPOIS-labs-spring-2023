    ...
    def tick(self) -> None:
        ...
        if not self.__finished:
            match (self.get_current_action()):
                ...
                case Action.WAIT:
                    self.__wait_timer = 4
                    logging.info('[%s] is waiting on station [%i], [%i] days left to wait...', self.__name,
                                 self.__location,
                                 self.__wait_timer)
            ...
            if self.__travel_timer <= 0:
                if self.next_station() == -1:
                    ...
                    self.__travel_timer = int(ceil(
                        self.__railroad.direct_path_length(self.get_location(),
                                                           self.next_station()) * self.travel_speed()))
            else:
