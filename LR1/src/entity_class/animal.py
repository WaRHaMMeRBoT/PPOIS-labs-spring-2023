...
class Animal(Entity):
    ...
    def die(self, game_map):
        ...
        game_map.get_entity_list().append(Corpse(self.coord2d, self.max_hp))

    def reproduction_event(self, game_map):
        if self.current_saturation < self.saturation_cap * 0.85:
            return

        self.reproduction_bar += 1

        entity = game_map.get_nearest_pair(self.coord2d, self.get_idf(), self.sex)
        if entity is None:
            return
    ...
    def wander(self, game_map):
        rx, ry = random.randint(0, game_map.width), random.randint(0, game_map.height)
        coords = game_map.get_nearest_free_tile(Coord2d(rx, ry))
        self.move_to(coords)
