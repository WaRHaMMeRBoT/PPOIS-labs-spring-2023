...
class Animal(Movable, Dieable, Aging, Eatable, GenderReproduction, Hunger, Powerful, ABC):
    ...
    def live_time_cycle(self) -> NoReturn:
        ...
        # TODO Search for food, chance to move in class Ocean

    def __find_position_in_ocean(self, ocean: Ocean) -> Tuple[int, int]:
        not_found_flag = True
        i = 0
        j = 0
        for sqrkm_line in ocean.sqrkms:
            j = 0
            for sqrkm in sqrkm_line:
                if self in sqrkm.creations:
            ...
            break
        if not_found_flag:
            raise ValueError(f"Creature with id {self.id} wasn't found in ocean")
        return i, j

    def move(self, ocean: Ocean) -> NoReturn:
        chance_to_move = randint(1, 3)
        if chance_to_move != 1:
            return
        if not isinstance(ocean, Ocean):
            raise TypeError
        vertical_pos, horizontal_pos = self.__find_position_in_ocean(ocean)
        vertical_shift = random.randint(-1, 1)
        horizontal_shift = random.randint(-1, 1)
        while vertical_shift == 0 or\
                horizontal_shift == 0 or\
                vertical_pos + vertical_shift >= ocean.vertical_length or\
                horizontal_pos + horizontal_shift >= ocean.horizontal_length:
            vertical_shift = random.randint(-1, 1)
            horizontal_shift = random.randint(-1, 1)
        ocean.sqrkms[vertical_pos][horizontal_pos].creations.remove(self)
        ocean.sqrkms[vertical_pos + vertical_shift][horizontal_pos + horizontal_shift].creations.append(self)
